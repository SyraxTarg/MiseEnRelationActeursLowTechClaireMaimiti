...
if ($idAnnonce) {
    ...
    if (isset($_GET['filtrer'])) {
        if (!isset($_SESSION['selectedFiltre'])) {
            $_SESSION['selectedFiltre'] = "none";
        }
        if ($typeFiltre !== 'none') {
            switch ($typeFiltre) {
                case "Avancees":
        ...
        $_SESSION['selectedFiltre'] = $typeFiltre;
        header("Location: index.php?page=mur&p={$_GET['p']}&typeFiltre={$_SESSION['selectedFiltre']}");
    }
    ...
    if (isset($_GET['p'])) {
        ...
        if (isset($_GET['filtrer'])) {
            echo "dsjfbjhs";
            if ($typeFiltre) {
                $annoncesPaginees = $annoncesManager->rechercheAnnoncesByType($offsets[$_GET['p']], $typeFiltre);
            } else {
                $annoncesPaginees = $annoncesManager->getTenPinnedAnnonces($offsets[$_GET['p']]);
            }
        }
